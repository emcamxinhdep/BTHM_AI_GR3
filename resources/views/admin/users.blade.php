                                        ...
                                        <div class=" col-sm-12 emphasis" style="display: flex; justify-content: end">
                                            @if ($user->isActive == 'Chưa kích hoạt')
                                                <button type="button" class="btn btn-primary btn-sm"
                                                    data-attr='{"patientId": "{{ $user->patientId }}", "action": "{{ route('admin.status-patient') }}"}'
                                                    id="btn-active">
                                                    <i class="fa fa-check"> </i> Kích hoạt
                                                </button>
                                            @endif
                                            <button type="button" class="btn btn-primary btn-warning"
                                                data-attr='{"patientId": "{{ $user->patientId }}", "action": "{{ route('admin.status-patient') }}", "status": "b"}'
                                                id="btn-ban"
                                            ...
                                            <button type="button" class="btn btn-primary btn-warning"
                                                data-attr='{"patientId": "{{ $user->patientId }}", "action": "{{ route('admin.status-patient') }}", "status": ""}'
                                                id="btn-unban"
                                                style="{{ $user->status !== 'b' ? 'display: none;' : '' }}">
                                            ...
                                            <button type="button" class="btn btn-primary btn-danger"
                                                data-attr='{"patientId": "{{ $user->patientId }}", "action": "{{ route('admin.status-patient') }}", "status": "d"}'
                                                id="btn-delete"
